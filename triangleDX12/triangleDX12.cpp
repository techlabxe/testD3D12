...
BOOL ResourceSetupDX12()
{
  ...
  descPipelineState = d3d12util::CreateGraphicsPipelineStateDesc(
    ...
    descInputElements, _countof(descInputElements)
    );
  ...
  // 頂点データの作成.
  vertexBuffer = d3d12util::CreateVertexBuffer(dxDevice.Get(), sizeof(triangleVertices), D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ);
  if ( !vertexBuffer ) {
